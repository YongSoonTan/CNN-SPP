...
choose one fold at a time for data augmentation, then run save()
'''
Dataset = 1
path = os.path.join('CrossVal', 'D'+str(Dataset)) 
...
Train =sio.loadmat(os.path.join(path, 'D'+str(Dataset)+'_'+fold+'_fold_train.mat'))
...
def save(Train_Images=Train_Images, Train_Labels=Train_Labels):
    ...
    sio.savemat('Augmented_D'+str(Dataset)+'_'+fold+'_fold_train.mat', {'trainImages':Train_Images, 'trainLabels2': Train_Labels},do_compression=True)
